...
# Función para calcular métricas y generar gráficos  
def calcular_metricas_y_graficos(datos, sucursal):  
    ...
    for producto in productos:  
        ...
        st.metric(  
            ...
            delta=f"{((precio_promedio_2024 / precio_promedio_2023) - 1) * 100:.2f}%"  
        )  
        st.metric(  
            "Margen Promedio",   
            f"{margen_promedio:.2f}%",   
            delta=f"{((margen_promedio_2024 / margen_promedio_2023) - 1) * 100:.2f}%"  
        )  
        st.metric(  
            "Unidades Vendidas",   
            f"{unidades_vendidas:,}",   
            delta=f"{((unidades_2024 / unidades_2023) - 1) * 100:.2f}%"  
        )  
